...
export default function YourAudio() {
  ...
  const handleSaveAudio = useCallback(async (blob, url) => 
  {
    ...
    try {
      ...
      if (response.ok) {
        ...
        del.addEventListener("click", () => handleDeleteAudioFromDb(`http://localhost:5000/deleteaudio/${res.filename}`,audio));
  ...
  const handleDeleteAudioFromDb = async (audioUrl,audio) => {
    try {
        ...
        const response = await fetch(`http://localhost:5000/deleteaudio/${filename}`, {
            method: "DELETE",
            headers: {
      ...
      >
        ...
        <div className="my-8">
          <button
            ...
            className={`mx-4 text-black font-medium  bg-green-600 btn-sm ${check === true ? "cursor-not-allowed	" : ""
              }`}
          ...
          <button
            disabled={check === true ? true : false}
            className={`mx-4 text-white font-medium bg-red-600 btn-sm ${check === true ? "cursor-not-allowed	" : ""
              }`}
        ...
        {check ? (
          <>
            ...
            <div className="h-5">
              {heading === '' && <p style={{ color: 'red' }}>Field is required</p>}
            </div>
          </>
        ) : null}
