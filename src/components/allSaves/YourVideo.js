    ...
    <>
      ...
      {capturing ? (
        <button className='mx-4 text-white font-medium  bg-red-600 btn-lg' onClick={handleStopCaptureClick}>Stop Capture</button>
          ...
          <button disabled={showCam===true ? false : true} className={`mx-4 text-black font-medium  bg-green-600 btn-lg ${showCam === false ? "cursor-not-allowed	" : ""}`} onClick={handleStartCaptureClick}>Start Capture</button>
        )}
      {recordedChunks.length > 0 && (
        <button onClick={handleDownload}>Download</button>
        )}
      </div>
      {showCam ? (<div><Webcam className='w-25'  audio={false} ref={webcamRef} /></div>):('')}
      {recordedChunks.length > 0 && (
        <div className='flex justify-center'>
